...
public class LoanController {
    ...
    public void requestLoan(@RequestBody final LoanRequest loanRequest, HttpServletRequest req){
    User  borrower =tokenValidationService.validateTokenAndGetUser(req.getHeader(HttpHeaders.AUTHORIZATION));
        loanApplicationRepository.save(loanApplicationAdapter.transform(loanRequest, borrower));
    }

    @GetMapping(value = "/loan/requests")
    public List<LoanApplication> findAllLoanApplications(HttpServletRequest req){
        tokenValidationService.validateTokenAndGetUser(req.getHeader(HttpHeaders.AUTHORIZATION));
        return loanApplicationRepository.findAll();
    }

    @GetMapping(value = "/users")
    public List<User> findUsers(HttpServletRequest req){
        tokenValidationService.validateTokenAndGetUser(req.getHeader(HttpHeaders.AUTHORIZATION));
    ...
    public void acceptLoan(@PathVariable final String loanApplicationId,HttpServletRequest req){

        User lender = tokenValidationService.validateTokenAndGetUser(req.getHeader(HttpHeaders.AUTHORIZATION));
